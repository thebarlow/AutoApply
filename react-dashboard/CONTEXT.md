...
- `structuralControls.jsx` — structural mutation controls: add list item, add custom section + fields, rename (double-click), reorder, remove (bordered confirm dismissed on outside pointer-down), visibility toggle (`VisibleToggle`, eye), and `LlmWriteToggle` (lock: may the LLM rewrite this field?). Each field exposes exactly two icons — **lock** (`llm_output`) and **eye** (`visible`) — plus an `llm_instructions` box shown when unlocked; persisted via the tree `PUT /api/config/profiles/{id}/tree`. Per-field context-injection (`llm_input`) is no longer surfaced in the UI.
