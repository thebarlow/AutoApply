...
- `fieldWidgets.jsx` — per-kind field renderers: `TextWidget`, `MarkdownWidget`, `BulletsWidget`, `TaglistWidget`. **#4:** `MarkdownWidget` has a pop-out (⤢) large-editor modal for long text values (no chip injection). **Taglist live match:** as the user types in the `TaglistWidget` "Add…" box, existing chips that partially (case-insensitively) match the query highlight while non-matches dim, surfacing duplicates/near-duplicates before adding; an exact case-insensitive duplicate shows an "Already in your list" hint and is blocked from being re-added. Client-side, generic across all taglists.
