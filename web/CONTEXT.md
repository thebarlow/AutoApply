...
└── routers/
    ...
    ├── credits.py           # GET /api/credits, GET /api/admin/system-balance; require_real_admin dependency (session-based admin gate, shared by admin.py/dev.py). Admin grants live in admin.py (POST /api/admin/users/{profile_id}/grant)
...
- **Generation is asynchronous (202 + SSE)** — `POST /{job_key}/generate/resume` and `/generate/cover` run a synchronous affordability pre-check (`_precheck_generate_affordability`, preserving the 402 insufficient-credits response), then `_spawn` the 30–60s LLM+PDF work on a daemon thread and return **202** immediately; results stream over SSE. This matches the existing feedback-endpoint pattern and avoids Railway's edge-proxy timeout (the old synchronous path tripped 502s on the generate call and on dashboard polls queued behind it). The billable gate+debit runs inside the background meter (`run_resume_generation`/`run_cover_generation`, `intake_pipeline.py`); the endpoint only pre-checks affordability.
...
- **Credits & Metering (prepaid fixed-unit pricing, 2026-07-16 rework — DONE)** — every billable action has a fixed integer price in credits (`core/pricing.py` `price_for(action)`: `intake=2, generate_fresh=4, regenerate=2, score/extract/resume_parse/ats/rematch/draft=1`, each `PRICE_<ACTION>`-overridable), debited **upfront** via `core.credits.debit_fixed` (one atomic conditional `UPDATE`, balance can never go negative) before the action body runs, and refunded (`refund_debit`) on any exception. `routers/credits.py`: `GET /api/credits` returns the caller's `{balance, rate, recent[]}` (last 20 ledger rows; `{balance:0, rate:0.0, recent:[]}` if no `Account` row). `require_real_admin` (a FastAPI dependency, defined in `routers/credits.py`; audit S4 — every admin endpoint across `credits.py`/`admin.py`/`dev.py` now uses it) resolves the **real** logged-in account from the session's `account_id` (not `current_profile_id`, which would resolve the *impersonated* tenant mid-impersonation) and 403s unless `is_admin`; outside production it falls back to the dev-tenant account — in production there is **no fallback**: a request with no session `account_id` gets 403 even if the outer auth gate were bypassed. It gates `GET /api/admin/system-balance` (reads the platform OpenRouter key's remaining balance via `LLM_API_KEY`, 502 on upstream failure, 503 if unset). `core.credits.InsufficientCredits` is registered in `web/main.py` as an exception handler returning **HTTP 402** `{error:"insufficient_credits", balance, price, action}` (the old `{floor}` payload is retired) — raised by `debit_fixed` inside `meter_action` (see `core/CONTEXT.md` → "Credits & Metering") when the balance can't cover the action's price.
  ...
  - **Billing-disabled startup warning** — `_warn_if_billing_disabled()` (`web/main.py`, called in `lifespan`) logs a WARNING + startup print when `APP_ENV=production` and `CREDIT_DEFAULT_RATE` resolves ≤ 0, since `meter_action` bills nothing at rate 0 (warn, not crash — a free period may be intentional, but never unnoticed).
...
| `POST` | `/api/jobs/{job_key}/generate/resume` | Synchronous affordability pre-check (402 on insufficient credits), then spawns background resume MD+PDF generation and returns **202**; results stream over SSE |
| `POST` | `/api/jobs/{job_key}/generate/cover` | Synchronous affordability pre-check (402), then spawns background cover-letter MD+PDF generation and returns **202**; results stream over SSE |
...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/dev/login` | **Non-production only** (404s when `APP_ENV=production`, where OAuth is mandatory). Establishes a local session without OAuth by setting `session["account_id"]` — the same key a real OAuth callback sets — to the sole account (or the admin account if several exist); returns `{account_id, profile_id, email}`, or 404 if no account exists. Exists because the identity gate `GET /api/me` (`web/auth/routes.py`) has **no dev bypass**, unlike the `current_profile_id` tenancy seam; the Playwright harness (`e2e/`) calls it in `global-setup` to obtain a `storageState`. |
