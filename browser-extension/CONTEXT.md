...
- **Ashby — functional but rough.** The application is **not wrapped in a `<form>`** (the `|| document.body` fallback is what makes enumeration work — do not remove it). ~49% of controls (checkboxes/radios) yield **no derivable label** (label falls back to the raw UUID field id); radio option-groups enumerate one entry *per option* sharing a field id, so `build_plan`'s field-id dedup collapses them to the first option's text (the real question is lost). **Fixed in this task:** the static `ashby` schema's field ids (`name`/`email`/`phone`) previously did not match the live DOM (`_systemfield_name`/`_systemfield_email`/`_systemfield_phone`), so contact fields both duplicated (matched again via dynamic enumeration under the real id, but routed to the essay bucket since `_canonical_for_enumerated` only matches exact `CANONICAL_FIELDS` keys) and failed to fill under the static-schema field id. `core/ats_schemas.py` now uses the real `_systemfield_*` ids so email/name/phone resolve and fill deterministically — verified by the `e2e/extension/` Ashby fixture case.
