...
### `form_fill.js` — the writer
`fillForm(plannedFields)` (`content/form_fill.js`) is the last leg of the pipeline: it takes the `ApplicationPlan.fields` array `injector.js` got back from `handleEnumerateForm` and writes values into the live DOM.
- **Status filter:** only fields with `status === "filled"` or `"drafted"` **and** a non-empty `value` are written; `"blank"`/`"unknown"` fields (including every EEO field, which is never LLM-inferred — see `core/application_classify.py`/`core/application_mapper.py`) are always skipped, so the writer can never fabricate a demographic answer even if a future bug changed the plan shape.
- **Control lookup:** `_findControl(field_id)` tries `[name="<field_id>"]` → `getElementById(field_id)` → `[aria-label="<field_id>"]`, in that order — so the plan's `field_id` must equal the live DOM's `name`/`id` attribute for a static-schema field to be found at all (see `core/ats_schemas.py` — a schema `field_id` that doesn't match the live DOM silently fails to fill; this bit the Ashby schema, see `e2e/extension/CONTEXT.md`).
- **Native-setter write:** `_setNativeValue()` calls the `HTMLInputElement`/`HTMLTextAreaElement` prototype's `value` setter (not a plain `el.value = ...` assignment) before firing `input`/`change` events, because React-controlled inputs (Ashby's `_systemfield_*` fields) ignore a direct property write — only the prototype setter is visible to React's change-detection.
- `<select>` fields match by option `value` or trimmed option text; checkbox/radio match by control `value`, or by the literal `"true"` **only** when the control's own `value` is `""`/`"on"`/`"true"` (boolean-style controls) — a literal-`"true"` planned value no longer matches arbitrary radio-group options that happen to lack a matching `value`.
- **Per-field error isolation:** each field's `_findControl`/`_writeValue` call is wrapped in its own try/catch, so one control throwing (e.g. a file input rejecting a programmatic `.value` assignment — greenhouse/lever/ashby static schemas all declare a required resume/resume_file field) no longer aborts the loop; subsequent fields still get written.
...
- **`http://localhost:8080/*` `host_permissions` entry must be reviewed/removed before store packaging.** Added to unblock the admin-only Live/Local server toggle (see "Resolved" below) and needed for the `e2e/extension/` Playwright harness, but it ships in the production manifest today. A published store listing has no business requesting permission to a developer's local dev server — either gate it out of the packaged build or drop it and find another way to smoke-test locally before submitting to Chrome Web Store / Firefox Add-on store. Track alongside the pinned-`key` item below.
- **Pin a Chrome `key` in `manifest.json` to stabilize the redirect URL.** — **RESOLVED.** A fixed `"key"` (base64 public key) is now in `manifest.json`, so load-unpacked yields a constant Chrome extension ID on every machine: `hhdmojkjnegdgdaacleldfgonicipnac`, redirect URL `https://hhdmojkjnegdgdaacleldfgonicipnac.chromiumapp.org/`. That URL must be present in `EXTENSION_REDIRECT_URLS` (local `.env` + Railway) or sign-in still 400s. The matching private key lives at `backups/extension_key.pem` (gitignored) — keep it for store publishing. Firefox is stable via `browser_specific_settings.gecko.id`.
...
### Application-plan enumeration — smoke test PARTIALLY RUN (2026-07-20); harness verified (2026-07-22)
**Correction (2026-07-22):** the note below originally said "Live-DOM enumeration + routing validated." That smoke test only injected/called `enumerateForm()`/`labelFor()` directly and *simulated* the engine's routing decisions by hand — it did **not** go through the real content-script invocation path (`_maybeEnumerateApplyForm()` on script load), which was broken at the time (see "Resolved" → "Form enumeration never actually ran at runtime") and wasn't fixed until the `setTimeout(_maybeEnumerateApplyForm, 0)` change. So this entry validated the enumeration/classification *logic* in isolation, not the live end-to-end trigger. The findings below (Greenhouse clean, Ashby rough, essay-bucket catch-all) still stand as logic-level observations.

**What IS now verified end-to-end (not live-site, but through the real content-script path):** the Playwright fixture harness in `e2e/extension/` (added for this task) drives the actual unpacked extension — real `injector.js` load order, the deferred `_maybeEnumerateApplyForm()` call, `FIND_STAGED_JOB` service-worker matching, the `application-plan` POST, and `fillForm()` writing the response — against static Greenhouse/Lever/Ashby fixtures. All three ATS cases pass: the canonical email field fills correctly on every one, including Ashby's React-controlled `_systemfield_email` (native-setter write confirmed working). See `e2e/extension/CONTEXT.md` for harness details and remaining classifier gaps (e.g. Lever's single `name` field / phone/location are not asserted since the classifier may not map every static field). Live-site (non-fixture) verification is still the "Full-pipeline steps still to verify manually" checklist below.
...
- **Ashby — functional but rough.** The application is **not wrapped in a `<form>`** (the `|| document.body` fallback is what makes enumeration work — do not remove it). The `_waitForFormReady` gate also used to require a `<form>`, which permanently blocked Ashby enumeration despite that fallback — now resolved (see Resolved: the gate accepts rendered input controls too). ~49% of controls (checkboxes/radios) yield **no derivable label** (label falls back to the raw UUID field id); radio option-groups enumerate one entry *per option* sharing a field id, so `build_plan`'s field-id dedup collapses them to the first option's text (the real question is lost). **Fixed in this task:** the static `ashby` schema's field ids (`name`/`email`/`phone`) previously did not match the live DOM (`_systemfield_name`/`_systemfield_email`/`_systemfield_phone`), so contact fields both duplicated (matched again via dynamic enumeration under the real id, but routed to the essay bucket since `_canonical_for_enumerated` only matches exact `CANONICAL_FIELDS` keys) and failed to fill under the static-schema field id. `core/ats_schemas.py` now uses the real `_systemfield_*` ids so email/name/phone resolve and fill deterministically — verified by the `e2e/extension/` Ashby fixture case.
